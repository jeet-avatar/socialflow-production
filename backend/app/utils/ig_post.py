...
def post_reel(video_url: str, caption: str, ig_account_id: str, access_token: str) -> dict:
    ...
    try:
        ...
        logger.debug(f"[ig] Creating container for {ig_account_id}...")
        r = requests.post(
        ...
        logger.debug(f"[ig] Container created: {creation_id}")

        # Step 2 — poll until FINISHED
        logger.debug("[ig] Waiting for Instagram to process...")
        for attempt in range(1, 61):
            ...
            logger.debug(f"[ig] Attempt {attempt}/60: {status}")
            if status == "FINISHED":
                break
            if status == "ERROR":
                logger.debug(f"[ig] Error details: {r.json()}")
        ...
        logger.debug(f"[ig] Published! media_id={media_id}")
        return {"success": True, "media_id": media_id}

    except Exception as e:
        logger.debug(f"[ig] Exception: {e}")
